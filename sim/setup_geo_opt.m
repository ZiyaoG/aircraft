function geodesic = setup_geo_opt(n,D,N,controller)
...
lb = -2*ones(n,D+1);
ub = 2*ones(n,D+1);
lb(1,:) = -pi/6*ones(1,D+1);             % pitch angle  
ub(1,:) = 2*pi*ones(1,D+1);                
lb(2,:) = -30*pi/180*ones(1,D+1);             % angle of attack
ub(2,:) = 100*pi/180*ones(1,D+1);      
lb(3,:) = -30*pi/180*ones(1,D+1);             % pitch rate
ub(3,:) = 100*pi/180*ones(1,D+1);   
lb = lb';lb = lb(:);
ub = ub';ub= ub(:);
