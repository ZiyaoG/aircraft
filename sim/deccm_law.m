function [u,Erem,x_nom,u_nom]= deccm_law(t,x,plant,controller,distLearned,distEst,distEst_errBnd)
persistent t_pre beq_pre copt_pre Erem_pre gamma_s1_Mx_Traj phi0ccmTraj phi0robustTraj phi0_phi1Traj phi1robustTraj gamma_s1_Traj% for debugging

if isempty(t_pre) || (t == 0 && t_pre ~=0)
    t_pre = -3;
    Erem_pre = Inf;
end
if isempty(gamma_s1_Mx_Traj) || isempty(phi0ccmTraj) || isempty(phi0robustTraj)
    gamma_s1_Mx_Traj = 0;
    phi0ccmTraj = 0;
    phi0robustTraj = 0;
    phi0_phi1Traj = 0;
...
% for debugging
gamma_s1_Traj = [gamma_s1_Traj,gamma_s(:,end)];
gamma_s1_Mx_Traj = [gamma_s1_Mx_Traj,gamma_s1_Mx(3)];
...
if controller.distEstScheme ~= 0
    phi0 = phi0+ gamma_s1_Mx*(plant.B*distEst);
    if controller.use_distEst_errBnd == 1
        phi0 = phi0 + norm(gamma_s1_Mx*plant.B)*distEst_errBnd; 
    end
end
% debugging
% phi0robustTraj = [phi0robustTraj, phi0];
% phi1robustTraj = [phi1robustTraj, gamma_s1_Mx*plant.B];
...
% phi0_phi1Traj = [phi0_phi1Traj,-u + u_nom];
% if phi0 <=0
%     u = [0;0];
% else
%     phi1 = gamma_s1_Mx*plant.B;
